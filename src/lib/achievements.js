...
export const ACHIEVEMENT_REGISTRY = [
  ...
  // ── Дуэли ──
  {
    slug: "first_duel",
    title: "Дуэлянт",
    desc: "Завершить 1 дуэль",
    flavor: "Перчатка брошена и поднята",
    icon: "Swords",
    color: "orange",
    reward: 15,
    check: (s) => (s.duelsCompleted || 0) >= 1,
    progress: (s) => ({ current: s.duelsCompleted || 0, target: 1 }),
  },
  {
    slug: "duel_master",
    title: "Мастер дуэлей",
    desc: "Победить в 10 дуэлях",
    flavor: "Вас уже боятся вызывать",
    icon: "Swords",
    color: "red",
    reward: 50,
    check: (s) => (s.duelsWon || 0) >= 10,
    progress: (s) => ({ current: s.duelsWon || 0, target: 10 }),
  },
  {
    slug: "undefeated",
    title: "Непобедимый",
    desc: "5 побед подряд в дуэлях",
    flavor: "Серия побед впечатляет",
    icon: "Shield",
    color: "indigo",
    reward: 100,
    check: (s) => (s.duelWinStreak || 0) >= 5,
    progress: (s) => ({ current: s.duelWinStreak || 0, target: 5 }),
  },
  ...
  // ── Гонки ──
  {
    slug: "first_race",
    title: "Гонщик",
    desc: "Финишировать в 1 гонке",
    flavor: "Адреналин — лучший допинг",
    icon: "Zap",
    color: "emerald",
    reward: 15,
    check: (s) => (s.racesFinished || 0) >= 1,
    progress: (s) => ({ current: s.racesFinished || 0, target: 1 }),
  },
  {
    slug: "race_winner",
    title: "Чемпион",
    desc: "Победить в гонке",
    flavor: "Первое место — только начало",
    icon: "Trophy",
    color: "amber",
    reward: 30,
    check: (s) => (s.racesWon || 0) >= 1,
    progress: (s) => ({ current: s.racesWon || 0, target: 1 }),
  },
  {
    slug: "speed_demon",
    title: "Демон скорости",
    desc: "Победить в 10 гонках",
    flavor: "Вы — машина для побед",
    icon: "Flame",
    color: "red",
    reward: 100,
    check: (s) => (s.racesWon || 0) >= 10,
    progress: (s) => ({ current: s.racesWon || 0, target: 10 }),
  },
  {
    slug: "podium_king",
    title: "Король подиума",
    desc: "Топ-3 в 20 гонках",
    flavor: "Подиум — ваш второй дом",
    icon: "Crown",
    color: "violet",
    reward: 75,
    check: (s) => (s.racesPodium || 0) >= 20,
    progress: (s) => ({ current: s.racesPodium || 0, target: 20 }),
  },
...
export const ICON_MAP_KEYS = [
  ...
  "MessageCircle", "Star", "Moon", "Swords",
];
